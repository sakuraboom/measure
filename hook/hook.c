...
//日志信息
#define LOGLEN 128
char log_pathname[LOGLEN];
char log_time[LOGLEN];
char log_fileop[LOGLEN];
...
void read_pagemap_and_measurement_process(void)
{
	...
	int flag;
	char measureresult[256];
	memset(measureresult,0, 256);
	...
	if(flag){
		sprintf(measureresult,"%s %s %s %d %s %s wrong\n",
					log_time, log_pathname, current->group_leader->comm, 
					current->tgid, log_fileop, processtexthash);
	}
	else{
		sprintf(measureresult,"%s %s %s %d %s %s right\n",
					log_time, log_pathname, current->group_leader->comm, 
					current->tgid, log_fileop, processtexthash);
	}
...
asmlinkage long new_write(const struct pt_regs * regs)
{
	if(!query_list(current->group_leader->comm)){
		...
		if(query_senlist(pathname)){
			printk("my_write!\n%s\n",pathname);
			char data[16] = {0};
			raw_copy_from_user (log_pathname, pathname, strlen(pathname));
			sprintf(data, "%d", current->tgid);
			if (test_unicast(data, strlen(data), 1) == 0)
			{
				wait_for_completion(&comp);
				sprintf (log_fileop, "write");
				printk("virstart:%#lx\nvirend:%#lx\nprocessname:%s\n", virstart, virend, current->group_leader->comm);
				read_pagemap_and_measurement_process();
			}
		}
...
static void nl_recv_msg(struct sk_buff *skb)
{
	...
	raw_copy_from_user (log_time, time, strlen (time));
